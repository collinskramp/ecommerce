    ...
    return (
        ...
        <div className={styles === 'grid' ? 'w-full relative group h-[210px] md:h-[270px] xs:h-[170px] overflow-hidden' : 'md-lg:w-full relative group h-[210px] md:h-[270px] overflow-hidden'}>
            <Link to={`/product/details/${p.slug}`}>
                <img className='h-[240px] rounded-md md:h-[270px] xs:h-[170px] w-full object-cover cursor-pointer' src={ p.images[0] } alt={p.name} />
            </Link>
     ...
     <div className='flex justify-start items-start flex-col gap-1'>
            <Link to={`/product/details/${p.slug}`}>
                <h2 className='font-bold hover:text-[#059473] transition-all cursor-pointer'>{ p.name }</h2>
            </Link>
